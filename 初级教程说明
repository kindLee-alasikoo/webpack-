...
现在打开public文件夹会发现多一个bundle.js index.html引入bundle.js就会执行你在app文件夹下的js文件命令了以上为webpack低级用法

利用webpack配置文件进行文件打包操作以及写入插件和加载器。创建文件：webpack.config.js 前端工作者一般会用config.js进行配置，比如最近的项目需要针对不同的危险级别进行颜色配置，我们可以在color.config.js这样写一个对象
...
学习一些简单的命令，写入package.json中（1）用npm start代替webpack命令
...
执行命令是否写错。下载有些慢，需要等待，这时候我们可以阅读下devServer的api，在配置文件下写入
...
步先，我今天知道啦：由于我们是用npm start来执行webpack打包，所以我们需要在package.json里面
...
只要点击保存，页面也会随之变化。下面教程说的是关于react的，我只弄过vue，个人感觉Vue更简单，创建一个Vue脚手架项目只有四个命令
